...
**To dplyr users**: There is one main difference in the current version of SparkR  we need to be careful about. To indicate a column `col` in a `SparkDataFrame` `df`, we should use `df$col` in most cases and sometimes can use `"col"`. A general principle is when it involves a condition on the column or multiple `SparkDataFrame`s, we should use the former syntax. For example,
...
We use the `SparkDataFrame` `carsDF` created above.

We can get basic information about the `SparkDataFrame`.
```{r}
carsDF
```
...
**To dplyr users**: One main difference SparkR has with dplyr is instead 

### Single Table Verbs

#### filter

`filter` allows you to select a subset of rows based on given conditions

Select only the "eruptions" column.
```{r}
head(select(carsDF, carsDF$mpg))
```
